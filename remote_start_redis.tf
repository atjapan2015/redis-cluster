...
resource "null_resource" "redis_master_create_cluster" {
  ...
  provisioner "remote-exec" {
    ...
    inline = [
      ...
      "sudo -u root /usr/local/bin/redis-cli --cluster create `cat /home/opc/master_list.sh` `cat /home/opc/replica_list.sh` -a ${random_string.redis_password.result} --cluster-replicas 1 --cluster-yes",
